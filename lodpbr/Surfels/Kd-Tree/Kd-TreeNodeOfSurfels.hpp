...
private:
  ...
  /// Node's coordinates
  typename Surfel::Box3 mWorld;

public:

  /// List of pointers to points contained in node (if it is leaf) or point that
  /// splits this node in two
  typename Surfel::VectorOfPointerSurfel 	   mListPtr;
  /// constructor
  KdTreeNode (const typename Surfel::Box3& w) : mWorld(w)
  {
  ...
  /// constructor
  KdTreeNode (KdTreeNode* f, const typename Surfel::Box3& w) : mFather(f), mWorld(w)
  {
	son[1] 		= 0;
	son[0] 		= 0;
    mSplitDimension = 0;
  ...
  const KdTreeNode* SearchLeaf (const typename Surfel::Pointer& p) const
  {
	  if (mListPtr.size() == 1)
  ...
  const KdTreeNode* Search (typename Surfel::Pointer p)
  {
	  if (IsLeaf())
	  {
		  return this;
  ...
  typename Surfel::Type EuclideanDistanceToBox (const typename Surfel::Point3& p) const
  {

	  typename Surfel::Point3 closestPoint = p; //closest point on box to p

	  if (p.x <= mWorld.xMin())
	  {
		  closestPoint = typename Surfel::Point3(mWorld.xMin(), closestPoint.y, closestPoint.z);
	  }
	  else if (p.x >= mWorld.xMax())
	  {
		  closestPoint = typename Surfel::Point3(mWorld.xMax(), closestPoint.y, closestPoint.z);
	  }

	  if (p.y <= mWorld.yMin())
	  {
		  closestPoint = typename Surfel::Point3(closestPoint.x, mWorld.yMin(), closestPoint.z);
	  }
	  else if (p.y >= mWorld.yMax())
	  {
		  closestPoint = typename Surfel::Point3(closestPoint.x, mWorld.yMax(), closestPoint.z);
	  }

	  if (p.z <= mWorld.zMin())
	  {
		  closestPoint = typename Surfel::Point3(closestPoint.x, closestPoint.y, mWorld.zMin());
	  }
	  else if (p.z >= mWorld.zMax())
	  {
		  closestPoint = typename Surfel::Point3(closestPoint.x, closestPoint.y, mWorld.zMax());
	  }
  ...
  typename Surfel::Type InsertNeighbor (const typename Surfel::Pointer& itemPtr,
										typename 	   Surfel::Type dist,
										typename 	   Surfel::KNearestMap& pKNearest, unsigned int k) const
  {
	  pKNearest.insert ( typename Surfel::KNearestPair (dist, itemPtr) );
  ...
  int KNearestLocalNeighbors (const typename Surfel::Pointer& p,
							  typename 		 Surfel::KNearestMap& pKNearest,
							  unsigned int 	 k) const
  {
	  int comps = 0;
	  typename Surfel::Type minDist;
	  ...
	  {
		  typename Surfel::Pointer q = mListPtr[i];

		  if (mListPtr[i]->Center() != p->Center())
		  { // Check if not trying to insert itself
			  typename Surfel::Type dist = p->Center().EuclideanDistance (q->Center());
			  ++comps;
  ...
  /// @param k_nearest Ordered set of nearest neighbors
  /// @param k Number of nearest neighbors to find
  int KNearestNeighbors (const typename Surfel::Pointer& p,
							   typename Surfel::KNearestMap& pKNearest,
						   unsigned int k) const
  {
	  int comps = 0;
	  ...
	  if (!IsLeaf()) // Descend to child nodes
	  {
		  ...
		  typename Surfel::Type dists[2] = {son[0]->EuclideanDistanceToBox (p->Center()), son[1]->EuclideanDistanceToBox (p->Center())};
		  comps += 2;
		  int order[2] = {0, 1};
		  ...
		  typename Surfel::Type best = pKNearest.begin()->first;
  ...
  void Insert (int level, const typename Surfel::Pointer& p)
  {
	  ...
	  { // leaf node
		  ...
		  if (Refine::Split (mWorld, mListPtr))
		  {
			  ...
			  typename Surfel::Type center = 0.5 * (mWorld.Max()[mSplitDimension] + mWorld.Min()[mSplitDimension]);
			  typename Surfel::Type minDist = HUGE;

			  typename Surfel::Pointer middleItem;
			  for (unsigned int i = 0; i < mListPtr.size(); ++i)
			  {
				  typename Surfel::Type dist = fabs (center - mListPtr[i]->Center()[mSplitDimension]);
...
//	  return  sum + PtrList.size();
  ...
  const typename Surfel::Pointer Element (int id) const
  {
    return mListPtr[id];
  }
  ...
  typename Surfel::Type SplitCoordinate(void)
  {
  ...
  const typename Surfel::Box3 Box(void) const
  {
	  return mWorld;
  }
 ...
 // ====================================== Only for Clustering ============================================//
  ...
  typename Surfel::Type Cost (typename Surfel::Pointer& p,typename Surfel::Pointer& q)
  {
	  typename Surfel::Type cost = p->Center().EuclideanDistance (q->Center());
	  return cost;
  }
  ...
  /// @param k Number of nearest neighbors to find
  /// @return Number of distance comparisons made
  int KNearestLocalNeighborsClustering ( typename Surfel::Pointer& p, typename Surfel::KNearestMap& pKNearest, unsigned int k)
  {
	  int comps = 0;
	  typename Surfel::Type minDist;
	  ...
	  for (unsigned int i = 0; i < mListPtr.size(); ++i)
	  {
		  ...
		  if (mListPtr[i]->Marked() == 0) // Test if the item has been add
		  {
			  if (mListPtr[i]->Center() != p->Center())
			  { // Check if not trying to insert itself
				  typename Surfel::Type dist = Cost(p,q);
				  ++comps;
				  if (dist < minDist || pKNearest.size() < k)
				  {
						  pKNearest.insert ( typename Surfel::KNearestPair (dist, mListPtr[i]) );
  ...
  /// @param k_nearest Ordered set of nearest neighbors
  /// @param k Number of nearest neighbors to find
  int KNearestNeighborsClustering ( typename Surfel::Pointer& p, typename Surfel::KNearestMap& pKNearest, unsigned int k)
  {
	  int comps = 0;
	  // Computes the distance to this node's itens
...
private :
  ...
  /// @return Left son world coordinates
	typename Surfel::Box3 LeftBox (void) const
	{
		typename Surfel::Box3 leftWorld;
		typename Surfel::Point3 pointMax;

		if (mSplitDimension == 0) // recompute x coordinate range
		{
			pointMax = typename Surfel::Point3 (mSplitCoordnate, mWorld.yMax(), mWorld.zMax());
		}
		else if (mSplitDimension == 1)
		{
			pointMax = typename Surfel::Point3 (mWorld.xMax(), mSplitCoordnate, mWorld.zMax());
		}
		else
		{
			pointMax = typename Surfel::Point3 (mWorld.xMax(), mWorld.yMax(), mSplitCoordnate);
		}

		leftWorld = typename Surfel::Box3 (mWorld.Min(), pointMax);
  ...
  /// @return Right son world coordinates
	typename Surfel::Box3 RightBox (void) const
	{
		typename Surfel::Box3 rightWorld;
		typename Surfel::Point3 pointMin;

		if (mSplitDimension == 0) // recompute x coordinate range
		{
			pointMin = typename Surfel::Point3 (mSplitCoordnate, mWorld.yMin(), mWorld.zMin());
		}
		else if (mSplitDimension == 1)
		{
			pointMin = typename Surfel::Point3 (mWorld.xMin(), mSplitCoordnate, mWorld.zMin());
		}
		else
		{
			pointMin = typename Surfel::Point3 (mWorld.xMin(), mWorld.yMin(), mSplitCoordnate);
		}

		rightWorld = typename Surfel::Box3 (pointMin, mWorld.Max());
