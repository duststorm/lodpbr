...
Como mencionado no capítulo \ref{cap:02}, modelos baseados em pontos complexos podem chegar a
milhões ou mesmo bilhões de amostras, como por exemplo no projeto do David Digital\cite{Levoy2000}. 
...
Neste capítulos serão apresentados alguns métodos de simplificação divididos em duas categorias:
...
avaliar a importância de cada ponto na nuvem de pontos. 
...
Linsen \cite{Linsen2001} defini a informação contida em um ponto como uma soma ponderada de fatores que inclui distância e curvatura entre outros, estimados pelos seus vizinhos. 
Em Alexa et al.\cite{Alexa2001} os pontos são removidos de acordo com a importância de sua contribuição
na representação da superfície dada por \texit{MLS} ( \texit{Moving Least Squares}\abbrev{MLS}{\texit{Moving Least Squares}}).
Kalaiah e Varshney
...
superfície determinada pela nuvem de pontos seja suave. Contudo, alguns modelos, como os de peças mecânicas por exemplo, contêm descontinuidades, geralmente bordas afiadas que separam a superfície em duas
partes distintas. Song e Feng \cite{Song2009} criaram um método de
simplificação considerando estes modelos baseado na mesma ideia do métodos
citados acima, porém com duas diferenças principais.
Primeiro, ao invés de operar sobre toda a nuvem de pontos de uma só vez,
o algoritmo considera apenas uma região suave distante por vez. 
...
continua, e Kalaiah e Varshney \cite{Kalaiah2003} na renderização e em
...
Os algoritmos citados acima criam uma nuvem de pontos simplificada realizando uma amostragem do modelo, ou seja, definindo um subconjunto da nuvem de pontos original ao remover iterativamente pontos de acordo com uma métrica de erro.
...
erros proposto por estes métodos por vezes não produzem um boa estimativa, não sendo trivial formular métricas mais precisas.
...
\begin{itemize}
  ...
  $\mathbf{\mathcal P}$ é dividida em subconjuntos. Os pontos deste subconjunto são então substituídos por um ponto representativo (geralmente seu centroide). Existem dois tipos de clusterização: incremental, onde clusters
...
vizinhos; e o hierárquico, onde clusters são criados tomando a nuvem de pontos original e dividindo-a em conjuntos menores.}
  ...
  de pontos e junta-os em um só. Note que para cada junção, há um acréscimo no
  erro total. No entanto, cria-se uma lista ordenada com todos pares possíveis, colocando no topo a junção que provoca o menor erro. Pares de pontos são então aglomerados de forma que o erro seja mínimo. Para avaliar o
  ...
  $|\mathbf{\mathcal{P}^}^\prime| = n$. Então cada ponto é considerado copmo uma
partícula que aplica forças de repulsão às vizinhas, espalhando-se até que a
...
pontos produzida pelos métodos de clusterização se apro'xima da distribuição
original, que é desejável em alguns caso. Entretanto, simulação de
partículas é a alternativa mais viável quando se deseja um densidade uniforme ou
um controle da densidade local da distribuição. Métodos de clusterização são os mais
eficientes devido a sua simplicidade. Clusterização incremental, simplificação iterativa e simulação de partículas requerem uma capacidade grande de memória por ser linear no número de amostras, enquanto a
clusterização hierárquica depende apenas do tamanho do modelo simplificado. O autor discute que as três técnicas operam diretamente na nuvem de pontos, ao invés de reconstruir a superfície a priori e só então aplicar a simplificação. A Figura~\ref{fig:pauly} ilustra estes métodos.
...
$\mathbf{\varepsilon}$. Para inicializar o algoritmo, em uma fase de pré-processamento, para cada ponto ${\mathbf{p}_i}$ propriedades locais da superfície como vetor normal $\mathbf{n}_i$ e 
densidade $\mathbf{\omega}_i$ são calculadas baseados em uma vizinhaça local. Dependendo da
...
A simplificação otimizada segue três passos. No primeiro passo é computado o
...
adapta-se melhor a curvatura da superfície. Em um segundo passo, para o conjunto
...
\textit{junção de} \texti{splats} são organizada em um lista de prioridades de
...
em um \textit{splat} $\mathbf{s}_m$. Assim,  a lista de prioridades conterá
todos as
