...
requer achar a célula que contenha $x$ e computar todas as células que
...
\textit{kd-tree} (direita) em $2D$. Células em destaque tem seus pontos testados
...
de normais, opcionalmente uma cor, além de alguns \textit{bits} para representam a estrutura da árvore. A hierarquia é criada em um pré-processamento
...
O algoritmo SPT é simples, de fácil implementação e provê uma renderização contínua usando
...
Outra desvantagem é que por não realizar \textit{frustum culling} o SPT pode perder em eficiência em alguns casos.
