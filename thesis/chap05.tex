...
\textit{splats} vizinhos. Este crescimento é guiado por dois erros: primeiro o
...
em \textit{splats} e que de inícios todos são \textit{splats} circulares. 
\section{Motivação}
...
representa-lo tornando operações de simplificação um ferramenta necessária para
torna possível viável algoritmos de modelagem e visualização.
Como mencionado no seção~\ref{cap02:sec:splat}, \texit{splats} são muitos
...
estruturas hierárquicas. Os pontos são organizados em estrutura de partição
espacial (ver capítulo~\ref{cap03}) e os \textit{splats} são criados pela
analise local da superfície. Estas técnicas são simples e rápidas, mas como não
há uma estratégias de otimização mais elaborada, e os resultados geralmente são
...
e incremental para renderização eficiente usando \textit{splats}. Clusters serão
criados adicionado vizinhos de acordo com métricas de erro que serão
apresentadas nas seções~\ref{cap05:sec:perpendicular}
e~\ref{cap05:sec:tangencial}.
 
\section{Clusterização Incremental}
Com uma semente $\mathbf{s}_j \in \mathbf{\mathcal T}$  escolhida
randomicamente, um cluster $\mathit{C_0}$ é construído adicionando-se 
sucessivamente os vizinhos mais próximos de $\mathbf{s}_j$. Este crescimento
incremental para quando um limitante $\varphi_p$ (para erro perpendicular) e 
$\varphi_t$ (para o erro tangencial) é alcançado. O próximo cluster
$\mathit{C_1}$ é então construído com o mesmo processo, com uma nova
semente escolhida da vizinhança de $\mathit{C_0}$.
De inicio cada vizinho que possui erro perpendicular abaixo de $\varphi_p$ é
...
$\mathbf{s}_m$ que representa cluster, é atualizado
(ver~\ref{cap05:sec:centroNormal}). Com $\mathbf{c}_m$ e $\mathbf{n}_m$,
podemos atualizar a extensões do \textit{splat} $\mathbf{s}_m$
(ver~\ref{cap05:sec:eixos}) e com isso computar o erro tangencial
$\mathbf{e}_t$. Se o erro tangencial calculado estiver acima de $\varphi_t$, a
configuração do ultimo vizinho adicionado é estabelecida como o cluster final.
O processo continua até que todos os \textit{splats} de  $\mathbf{\mathcal T}$
estiverem em algum cluster $\mathit C_i$.
...
Similar a~\ref{cap04:sec:splats:l21}, afim de obter os eixos da elipse
...
principais da elipse obtemos, assumindo que a excentricidade da elipse e dada
...
\langle t_{i}^{2},{n}_m\rangle^{2}}+|\langle c_{i}- c_{m},{n}_m\rangle|\\
\end{eqnarray*}
Podemos utiliza $\max_{i}\{\mathbf{\mathcal D}(i,m)\}$ como medida do erro
...
a encontrar o ponto $q$ e $\mathbf{s}_{m}$ mais afastado de $\Gamma$. 
...
\begin{enumerate}
 ...
 n_{x}$ é o número de intersecções entre elipses de $\Gamma$, o qual já é quadrático
 no número dessas elipses. Além disso o simples cômputo de uma intersecção entre duas elipses em posição genérica já envolve 
...
\{p_1,\ldots,p_8\}$, de $8$ de seus pontos. Especificamente,
...
Utilizando a distancia aproximada definida acima se obtém para cada $p_{k}$
...
\section{Resultados}
Abaixo uma tabela com o resultado da simplificação de $3$ modelos. O
limitante para erro perpendicular $\varphi_p$ foi tomado como sendo o dobro da
distância do vizinho mais próximo à semente $dist(\mathbf{s}_{seed},p_1)\quad
p_1 \in \mathbf{\mathcal{N}}_k(\mathbf{s}_{seed})$ . O limitante tangencial $\varphi_t$ foi
tomado como sendo a dimensão do eixo menor da semente $||\mathbf{t}_{seed}^1||$.
\begin{table}[hb]
\begin{center}
\begin{tabular}{lcccc}
\hline
Modelo  & N° \textit{Splats} Originais &$\varphi_p$ & $\varphi_t$ &
N° \textit{Splats} Simplificação
\\
\hline
\textit{Bunny} 	   & $139990$ & $2\cdot dist(\mathbf{s}_{seed},p_1)$   &
$||\mathbf{t}_{seed}^1||$ & $11258$\\
\hline
\textit{Armadillo} & $172974$ & $2\cdot dist(\mathbf{s}_{seed},p_1)$   &
$||\mathbf{t}_{seed}^1||$ & $11964$\\
\hline
\textit{Dragon} & $437645$ & $2\cdot dist(\mathbf{s}_{seed},p_1)$   &
$||\mathbf{t}_{seed}^1||$ & $37959$\\
\hline
\end{tabular}
\end{center}
...
\end{minipage}
\end{tabular}
\caption{\textbf{(a)} Modelo \textit{Bunny} original: $139990$.
\textbf{(b)} \textit{Bunny} simplificado: $11258$. \textbf{(c)} Modelo
\textit{Armadillo} original: $172974$. \textbf{(d)} Modelo simplificado:
$11964$.\textbf{(e)} Modelo \textit{Dragon} original: $437645$ 
\textbf{(f)} Modelo simplificado $37959$.}
\label{fig:pauly}
\end{figure}

Foi usada uma \textit{K-D Tree} para computar os $k-$vizinhos mais próximos da
semente. Foram usados $32$ vizinhos mais próximos, mas este número depende
da densidade do modelo. Sendo assim  o tamanho do cluster fica limitado a
$32$, mas nem sempre são tomados os $32$ vizinhos.
...
elíptico, o erro perpendicular e o tangencial. 
