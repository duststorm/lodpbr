...
MyMainWindow::MyMainWindow (QMainWindow *parent): QMainWindow(parent)
{
    ...
    OpenGLScene         * openglScene        = new OpenGLScene();

    ContextGraphicView  * contextGraphicView = new ContextGraphicView();

	contextGraphicView->setScene(openglScene);
	contextGraphicView->setViewport(new QGLWidget);
	contextGraphicView->setViewportUpdateMode(QGraphicsView::FullViewportUpdate);

   //	setCentralWidget(contextGraphicView);

		 QMdiSubWindow *subWindow1 = new QMdiSubWindow;
	     subWindow1->setWidget(contextGraphicView);
