...
*/
	...
	inline void Quaternion<Real>::To4x4Matrix( Real * rotationMatrix ) const
	{
	    ...
	    rotationMatrix[3] = xy+zw;
	    rotationMatrix[4] = static_cast<Real>(1.0) - (xx+zz);
	    rotationMatrix[5] = yz-xw;

	    rotationMatrix[6] = xz-yw;
	    rotationMatrix[7] = yz+xw;
	    rotationMatrix[8] = static_cast<Real>(1.0) - (xx+yy);

	    rotationMatrix[9] = rotationMatrix[1].w = rotationMatrix[2].w = static_cast<Real> (0.0);

	    rotationMatrix[10] = rotationMatrix[3].y = rotationMatrix[3].z = static_cast<Real> (0.0);
