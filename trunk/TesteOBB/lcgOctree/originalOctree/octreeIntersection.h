...
template <class K>
  double distance2Item (const Facet_handle* f, const CGAL::Point_3<K>& p, CGAL::Point_3<K>& clsp)
  {
    ...
    if(pointInTriangle(p_proj, tri)){
    	b = to_double(squared_distance(p, tri.supporting_plane()));
    }else{
    	CGAL::Segment_3<K> edge1 (tri[0], tri[1]);
    	CGAL::Segment_3<K> edge2 (tri[1], tri[2]);
    	CGAL::Segment_3<K> edge3 (tri[2], tri[0]);
    	double b1, b2, b3;
    	b1 = to_double(squared_distance(p, edge1));
    	b2 = to_double(squared_distance(p, edge2));
    	b3 = to_double(squared_distance(p, edge3));
    	if((b1 < b2) && (b1 < b3)){
	...
	double cos_ang = to_double((v_dist * plane_normal))/(norm(v_dist) * norm(plane_normal));
...
inline double norm(CGAL::Vector_3<Kernel> v){
   	return (sqrt(to_double(v[0]*v[0] + v[1]*v[1] + v[2]*v[2])));
}
