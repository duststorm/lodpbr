...
Box_3 coords (int sonIndex) const {
     ...
     double middle[3] = {to_double((p1[0] + p2[0]))/2,
				     to_double((p1[1] + p2[1]))/2,
					to_double((p1[2] + p2[2]))/2};

     int mult = 1;
     for (int d = 0; d < 3 ; d++){
         if ((sonIndex & mult) == 0) {
             son_min[d] = to_double(p1[d]);
             son_max[d] = middle[d];
         }
         else {
             son_min[d] = middle[d];
             son_max[d] = to_double(p2[d]);
         }
         mult *= 2;
     }
...
double distance(const Point3& p) const
{
    ...
    {
        if (p[i] < this->min()[i])
            clsPt[i] = to_double(this->min()[i]);
        else if (p[i] > this->max()[i])
            clsPt[i] = to_double(this->max()[i]);
        else
            clsPt[i] = to_double(p[i]);
    }
