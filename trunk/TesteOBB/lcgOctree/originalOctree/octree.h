...
public:
    ...
    virtual const OctreeNode* searchLeaf (const Box3& world, const Point3& p) const {
        ...
        for (int d = 0; d < 3; ++d) {
            double middle = to_double((p1[d] + p2[d])) * 0.5;
            if (to_double(p[d]) > middle) {
                index += mult;
                min[d] = middle; max[d] = to_double(p2[2]);
            } else {
                max[d] = middle; min[d] = to_double(p1[d]);
            }
            mult *= 2;
        }
...
public :
    ...
    Box3 box (const Iterator& i) const {
        double xsize = to_double(world.xmax() - world.xmin());
        double ysize = to_double(world.ymax() - world.ymin());
        double zsize = to_double(world.zmax() - world.zmin());
        double x0, y0, z0;
