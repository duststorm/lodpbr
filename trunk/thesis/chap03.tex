...
Estruturas de Partição do Espaço (EPE) são comuns em computação gráfica, em
particular quando se deseja processar modelos como os gerados por
\textit{scanners 3D}\cite{Levoy2000}. 
Métodos de simplificação, reconstrução,compressão, visibilidade, e outros
são beseados nesse tipo de estrutura. O objetivo de um estrutura de partição do
espaço é criar uma indexação do espaço, ou seja, dividi-lo em células e provê uma relação entre estas 
e o espaço ocupado por um objeto \cite{Samet2005}. Há vários algoritímos
disponpíveis para estruturar nuvem de pontos, baseado em representações
hierarquicas e não hierarquicas ou em cluster de pontos. Nesta sessão as
...
Hierarquia de Volumes Envolventes (Bounding Volume Hierarchies - BVH) (sessão \ref{section:BVH}).
...
tratar grandes modelos de pontos em especial, quando se quer renderizá-los de
forma interativa
\cite{BotschW2002,PajPRT2004,EfficenteLOD2003,PajConfetti2004}. A estrutura
é usada para particionar a caixa envolvente $3D$ que engloba todas as amostras no espaço. Cada célula  que contenha amostras é recursivamente subdivida em oito octantes.A recursão termina quando em uma
...
Uma \textit{K-d Tree} é em geral , um árvore multidimensional de busca em $k$
dimensões. Para dados em $3D$ a árvore correspondente é geralmente chamada de
\textit{K-d Tree} tridimensional ao invés de \textit{$3-d$ Tree}. Elas são um
caso especial de arvores partição espacial binária (BSP Tree - Binary Space Partitioning Tree)
\abbrev{BSP Tree}{\textit{Binary Space Partitioning Tree}} . \textit{K-d
Tree} usa planos de cortes que são perpendicular a um dos eixos coordenados ( também chamado hiperplanos), que é uma
especialização de uma $PBE$, em que planos de cortes arbitrários podem ser
usados. Um conjunto de pontos em uma \textit{K-d Tree} é subdividido em caixas
...
indexação no espaço, como \textit{Octrees} e \textit{K-d Trees} descritas
