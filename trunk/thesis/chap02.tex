...
\label{cap:02}
Superfícies baseadas em pontos ganharam recentemente a atenção da
...
de normal, cor e um coeficiente de transparência, além de um valor que estima a 
...
renderização. (\emph{b}) \textit{z-buffer} com tolerância para reconstruir uma
superfície contínua.}
...
No capítulo \ref{cap03} será apresentada uma estrutura eficiente para obter
os 
...
simétrica, ela possui todos os seus autovalores reais. Desta forma pode-se
...
porém algumas regiões podem ficar super-amostradas, já que áreas planares
...
Weighted Average}}) para reconstruir a superfície em espaço de imagem utilizando os \textit{splats}
projetados.
...
tangentes à superfície (Figura \ref{fig:bartellipse}), onde os tamanhos
...
se interseptam em espaço de imagem pertencem de fato à mesma superfície do
...
preenchendo o buffer de profundidade de modo habitual; e na segunda as elipses são
interpoladas utilizando o z-buffer da primeira passada como referência.
...
iluminação é realizado por pixel da imagem, uma técnica conhecida como
