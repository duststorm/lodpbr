...
coordenadas $X$, $Y$ e $Z$) é algum múltiplo de $1\13$. Para garantir
...
anglo de abertura do cone são $1\16$, $4\16$, $9\16$ e
$16\16$ }
...
\textit{QSplat} possui um processamento de dados muito simple e de fácil
implementação, mas infelizmente sua estrutura hierárquica recursiva é dificil de
...
geométrico:
Agora o erro no espaço de image $\tilde{e}_g$ depende apenas de $r$, e não mais
...
Um objeto é renderizado na hierarquia de pontos usando um percorrimento em
profundidade. Para cada nó um erro de imagem $\tilde{e}_g$ é calculado. Se
$\tilde{e}_g$ está abaixo de um limite de error estabelecido $\epsilon$ e o nó
...
mas também para propriedades da superfície. Grandes áreas planas são detectadas
...
processamento to rápido e sequêncial da GPU. Assim, há um rearranjamento da
estrutura em árvore para um estrutura em lista e o teste recursivo é
substituido por um percorrimento sequêncial sobre a lista de pontos.

Para isso, o erro simplificado $\tilde{e}_g$ é subistituido por um que sejem
...
teste recursivo para $r > r_{\min}$. Entretando, quando os nós da árvore são
processados sequencialmente sem informação hierarquica, há necessidade de um
teste não recursivo. Para esse fim, é adicionado um parâmetro $r_{\max}$, que é
simplesmnete um $r_{\min}$ do seu nó pai, em cada nó e usa-se $r \in
[r_{\min},r_{\max}]$ como um teste não recursivo. Desta maneira pode-se guiar o
algoritmo de renderização com esse teste \textit{intervalar} para cada nó.
...
O autor da ênfanse no fato de que grande parte do trabalho é movido para GPU,
