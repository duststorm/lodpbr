...
\section{Discussão}
A aquisição de modelos de pontos (via $3D$ \textit{scanners}) produz amostras
 alta densidade o que torna difícil um processamento eficiente deste
 ...
 vários técnicas de simplificação de modelos, considerando apenas pontos.
 Enquanto estas técnicas são eficientes, elas possuem um desvantagem de produzirem uma
 aproximação muito simples. Um representação mais elaborada baseada em
 \textit{splats} é uma extensão popular de superfícies baseada em pontos,
 principalmente quando se deseja visualização eficiente e com qualidade. Poucos
 algoritmos utilizam toda a geometria dos \textit{splats} quando aplicam
 técnicas de simplificação. Simplificação considerando \textit{splats} evita um
 posterior processo para obtenção de \textit{splats}, porém são um pouco mais
 elaborados. No capítulo a seguir será apresentado uma simplificação
