...
partes distintas. Song e Feng \cite{Song2009} resolveram este problema criando um método de
...
vizinhos; e o hierárquico, onde clusters são criados tomando a nuvem de pontos original e dividindo-a em conjuntos menores.}
  ...
  de pontos e juntando-os em um só. Note que para cada junção, há um acréscimo no
...
densidade $\mathbf{\omega}_i$ são calculados baseados em uma vizinhaça local. Dependendo da
...
de acordo com sua distâncias. Esta distância é calculada usando \ref{eq:circular} para
\textit{spalt} circular ou \ref{eq:eliptico} para elíptico, sendo que este último
adapta-se melhor à curvatura da superfície. Em um segundo passo, para o conjunto
...
onde um índice ${i}$ refere-se a um ponto $\mathbf{p}_i$. 
...
quando projetados em $2D$ no plano definido por $\mathbf{s}_m$ (Figura~\ref{fig:merge}).
...
porém não mantém a manter informação de topologia.
